...
def test6():
    ...
    for i in range(0,1000,1):
        ...
        yPos = int(10000.0*np.sin(2.0*np.pi*i/100))
        print(xPos, flush=True)
        joystickPosition = vj.generateJoystickPosition(wAxisXRot = 16000+xPos, wAxisYRot = 16000+yPos)
        vj.update(joystickPosition)
        time.sleep( 0.01 )
    joystickPosition = vj.generateJoystickPosition(wAxisXRot = 16000, wAxisYRot = 16000)
    vj.update(joystickPosition)
    vj.sendButtons(0)
    print("vj closing", flush=True)
    vj.close()
